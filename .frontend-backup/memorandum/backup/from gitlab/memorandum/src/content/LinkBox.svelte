<script>
    ...
    function dragStartFolder(event) {
        ...
        let originIndex = [].slice.call(event.target.parentNode.children)
    ...
    function dropFolder(event) {
        ...
        if(originIndex !== targetIndex) {
            ...
            currPreset.Folders.splice(
                targetIndex, 0, currPreset.Folders.splice(originIndex, 1)[0]);
    ...
    function dragStartLink(event) {
        ...
        if(event.target.nodeName !== "DIV"){ 
            originFolderIndex = [].slice.call(event.target.parentNode.parentNode.parentNode.parentNode.children)
                .indexOf(event.target.parentNode.parentNode.parentNode);
            originLinkIndex = [].slice.call(event.target.parentNode.parentNode.children)
                .indexOf(event.target.parentNode);
        } else {
            originFolderIndex = [].slice.call(event.target.parentNode.parentNode.parentNode.children)
                .indexOf(event.target.parentNode.parentNode);
            originLinkIndex = [].slice.call(event.target.parentNode.children)
                .indexOf(event.target);
        }  
    ...
    function dropLink(event) {
        ...
        currPreset.Folders[targetFolderIndex].links.push(
            currPreset.Folders[originFolderIndex]
                .links.splice(originLinkIndex, 1)[0]
        );
    ...
    function _getTargetIndex(event) {
        ...
        if(event.target.nodeName === "A" || 
            ...
            targetIndex = [].slice.call(event.target.parentNode.parentNode.parentNode.parentNode.children)
                .indexOf(event.target.parentNode.parentNode.parentNode);
        } else {
            targetIndex = [].slice.call(event.target.parentNode.parentNode.children)
                .indexOf(event.target.parentNode);
        }
