...
- [todo] Replace raw mobile-edit button with Carbon ghost Button; edit button now always visible directly before delete button; add subtle dark row background with hover highlight; add `--default_padding` gap between edit and delete buttons; indent category headers by `--default_padding / 2`.
...
- [about] Replace HTML entities (`&lpar;`, `&lcub;`, `&gt;` etc.) in code blocks with actual characters — CodeSnippet renders plain text, entities were displayed literally.
