...
export class TodoService {
  ...
  async getSharedTodoList(
    ...
    const result = await this.todoMongoDbService.findOne(getSharedTodoListInput.id);
    if (!result) {
  ...
  async removeSharedTodoList(
    ...
    const result = await this.todoMongoDbService.remove(removeSharedTodoListInputDto.id);
    if (!result) {
